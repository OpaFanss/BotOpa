/*
   ...
   Sc OpaFanss Akan Terus Di Update.
...
caption: respon,
sequenceNumber: 1656662972682001, timeOffset: 8600, jpegThumbnail: fs.readFileSync('./media/OpaFanss.jpg'),
contextInfo: {
...
break
            ...
            case 'gimage': {
                if (!text) return newReply(`Contoh : ${prefix + command} OpaFanss`)
                newReply(mess.wait)
                if (global.db.data.users[m.sender].limit < 1) return newReply(mess.endLimit) // respon ketika limit habis
                db.data.users[m.sender].limit -= 5 // -1 limit
...
break
            ...
            case 'attp':
                try {
                if (args.length == 0) return newReply(`Example: ${prefix + command} OpaFanss`)
            ...
            let media = await conn.downloadMediaMessage(qmsg)
            let audio = await toAudio(media, 'mp4')
            conn.sendMessage(m.chat, {document: audio, mimetype: 'audio/mpeg', fileName: `Convert By OpaFanss.mp3`}, { quoted : m })
        ...
        break
	        ...
	        case 'carbon':
	            ...
	            newReply(mess.wait)
	            conn.sendMessage(m.chat, { image: { url: `https://api.lolhuman.xyz/api/carbon?apikey=${apikey}&code=${q}&language=nodejs`}, caption: `Created By OpaFanss\n\n\nCode:\n\n${q}`}, {quoted: m})
	        break
		...
		case 'freefire':
			if (args.length == 0) return reply(`Example: ${prefix + command} OpaFanss `)
			if (global.db.data.users[m.sender].limit < 1) return newReply(mess.endLimit) // respon ketika limit habis
            db.data.users[m.sender].limit -= 2 // -1 limit
			newReply(mess.wait)
			conn.sendMessage(m.chat, { image: { url: `https://api.lolhuman.xyz/api/ephoto1/${command}?apikey=${apikey}&text=${args}`}, caption: `Created By OpaFanss\n\n Type: ${command}\n\nText: ${args}`})
			break
		...
		case 'carvedwood':
            ...
            db.data.users[m.sender].limit -= 2 // -1 limit
			newReply(mess.wait)
			conn.sendMessage(m.chat, { image: { url: `https://api.lolhuman.xyz/api/photooxy1/${command}?apikey=${apikey}&text=${args}`}, caption: `Created By OpaFanss\n\n Type: ${command}\n\nText: ${args}`})
			break
    ...
    // Text Prome
		...
		case 'wallgravity':
			...
			newReply(mess.wait)
			conn.sendMessage(m.chat, { image: { url: `https://api.lolhuman.xyz/api/textprome2/${command}?apikey=${apikey}&text1=${args[0]}&text2=${args[1]}`}, caption: `Created By OpaFanss\n\n Type: ${command}\n\nText: ${args}`})
			break
		...
		case 'thunder':
            ...
            db.data.users[m.sender].limit -= 2 // -1 limit
			newReply(mess.wait)
			conn.sendMessage(m.chat, { image: { url: `https://api.lolhuman.xyz/api/textprome/${command}?apikey=${apikey}&text=${args}` }, caption: `Created By OpaFanss\n\n Type: ${command}\n\nText: ${args}`})
			break
        ...
        break
            ...
            if (!text) return reply(`Example: ${prefix + command} OpaFanss`)
